...
public sealed class PrintManager
{
        ...
        string? fileName = null)
    {
        ...
        PrintLayout layout = _layoutEngine.CalculateLayout(pageSettings, font);

        string[] allLines = GetAllLines(buffer);
        int[] lineLengths = allLines.Select(l => l.Length).ToArray();

        List<PageRange> pages = _layoutEngine.CalculatePageBreaks(
    ...
    private void OnPrintPage(object? sender, PrintPageEventArgs e)
    {
        ...
        if (_layout is null || _pages is null)
        {
            ...
            _layout = _layoutEngine.CalculateLayout(pageSettings, _font);

            string[] allLines = GetAllLines(_buffer);
            int[] lineLengths = allLines.Select(l => l.Length).ToArray();

            _pages = _layoutEngine.CalculatePageBreaks(
                allLines.Length, _layout.LinesPerPage, _wordWrap, lineLengths, _layout.CharsPerLine);
        }
